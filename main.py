...
def main():
    """
    Função principal para coleta de dados de vídeos do YouTube.
    ...
    try:
        ...
        for i in range(num_videos):
            try:
                logger.info(f"\n{'='*60}")
                logger.info(f"VÍDEO {i+1}/{num_videos}")
                logger.info('='*60)
                ...
                if isinstance(data_comments, dict) and "error" in data_comments:
                    logger.warning(f"⚠️  Não foi possível coletar comentários: {data_comments['error']}")
                    data_comments = []
